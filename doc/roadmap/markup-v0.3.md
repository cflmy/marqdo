# 路线图：Marqdo Markdown 语法映射 v0.3（叙述统一）

「Markup」= Marqdo 把 Markdown 标记当成语法的那一层（不是另一个产品）。v0.3 = 现行映射：粗体代码、斜体返回。
...
- [x] 诊断：必选缺失、未知 callee（已有 errors 金样）；死变量不报错  
- [x] 残缺粗体：与 GAP-06/07「未闭合后恢复」冲突，保持 Comment 跳过恢复，不硬报错  
- [x] 粗体内表达式语句：`**n + 1**` 求值后丢弃（非误判为调用）  
- [x] 叙述斜体：仅整行 / `返回*…*` / 表达式形当返回（`*very important*` 不收束函数）  
