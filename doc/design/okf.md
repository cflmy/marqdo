...
4. **近义（Accepted，默认开）**：仍 miss 且 `near_match=True` 时，对 Task `title|description|aliases` 做本地**字符 bigram 词袋 + 余弦**；`best.score >= near_threshold`（默认 `0.78`）则 `match: near` 并带 `score`，`cache=soft-hit`，并可 `agent_kb_add_alias`。这是稀疏词法近邻，**不是** embedding 模型。  
5. **父裁决 soft_match（Accepted，默认关）**：仍 miss 且 `plan soft_match=True` 时，把 `agent_kb_near_match` 排序候选（含 score；空则退回 `list_tasks`）交给父模型：`DECISION: REUSE` + `SLUG:` 或 `DECISION: NEW`；REUSE 后可 `agent_kb_add_alias`。  
6. **不做**默认 dense embedding / 远程向量库。远期 dense 仍见 [okf-near-match.md](../roadmap/okf-near-match.md) E。
...
| `soft_match` / `软命中` | `False` | miss 后父裁决 REUSE/NEW（须显式打开；候选为 near 排序列表） |
| `near_match` / `近义命中` | `True` | lookup 第四趟本地 n-gram；`False` 关闭 |
| `near_threshold` / `近义阈值` | `0.78` | near 自动命中最低余弦分 |
...
| **O5** | 相近任务软命中：aliases + canonicalize + 本地 n-gram `near` + `soft_match` 父裁决已落地；dense embedding 仍见 [okf-near-match.md](../roadmap/okf-near-match.md) E | **A/B/C/D-near done** |
