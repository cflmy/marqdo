...
| 作者程序用 `[k](c)` 取元；`- [项](集合)` foreach | 新代码写 `` m[^k] `` |
| 中英 API 对称（见 [stdlib-i18n.md](stdlib-i18n.md)） | 只改一侧 |
| 一例跑通即文档（public/stdlib 或 examples） | 另开第二套「文档站」手写说明 |

**库函数体内的叙述坑：** 函数体里的链接形 `[k](c)`、以及单独的 `**强调**` 会被当成可执行代码。库内说明用「链接取元 / link-index」等字面描述；可执行示例放在 `public/stdlib` 或 `examples/`，不要写在 `##` 体内当叙述。
