...
func insertIgnoreSQL(dbURL, table, cols, placeholders, conflict string) string {
	if db.IsPostgres(dbURL) {
		return fmt.Sprintf(`INSERT INTO %s(%s) VALUES(%s) ON CONFLICT %s DO NOTHING`, table, cols, placeholders, conflict)
	}
...
func EnsureSchema(dbURL string) error {
	...
	if db.IsPostgres(dbURL) {
		for _, stmt := range schemaSQLPostgres {
			if _, err := sqlDB.Exec(stmt); err != nil {
				return err
			}
		}
	} else if _, err := sqlDB.Exec(schemaSQLSqlite); err != nil {
		return err
	}
	permSQL := insertIgnoreSQL(dbURL, "web_permissions", "code, resource, action, description", "?,?,?,?", "(code)")
	for _, p := range DefaultCatalog() {
		_, _ = sqlDB.Exec(permSQL, p.Code, p.Resource, p.Action, p.Description)
	}
	if err := ensureSystemRole(sqlDB, dbURL, "superadmin", true, AllCodes()); err != nil {
		return err
	}
	if err := ensureSystemRole(sqlDB, dbURL, "member", true, []string{"comments:create", "comments:delete_own"}); err != nil {
		return err
	}
	return nil
}

func ensureSystemRole(sqlDB db.SQLDB, dbURL, name string, isSystem bool, perms []string) error {
	var id int64
	...
	if err == sql.ErrNoRows {
		...
		if db.IsPostgres(dbURL) {
			err = sqlDB.QueryRow(`INSERT INTO web_roles(name, is_system) VALUES(?,?) RETURNING id`, name, sys).Scan(&id)
			if err != nil {
				return err
			}
		} else {
	...
	rpSQL := insertIgnoreSQL(dbURL, "web_role_permissions", "role_id, permission_code", "?,?", "(role_id, permission_code)")
	for _, code := range perms {
		_, _ = sqlDB.Exec(rpSQL, id, code)
	}
	return nil
}
...
func AssignRole(dbURL, username, roleName string) error {
	...
	_, err = sqlDB.Exec(
		insertIgnoreSQL(dbURL, "web_user_roles", "user_id, role_id, tenant_id", "?,?,?", "(user_id, role_id, tenant_id)"),
		uid, rid, "",
	)
	return err
}
...
func CreateRole(dbURL, name string) (int64, error) {
	...
	if db.IsPostgres(dbURL) {
		var id int64
		err = sqlDB.QueryRow(`INSERT INTO web_roles(name, is_system) VALUES(?,0) RETURNING id`, name).Scan(&id)
		return id, err
	}
...
func CreateUser(dbURL, username, passwordHash string) (int64, error) {
	...
	if db.IsPostgres(dbURL) {
		var id int64
		err = sqlDB.QueryRow(
			`INSERT INTO web_users(username, password_hash, created_at) VALUES(?,?,to_char(now() AT TIME ZONE 'UTC','YYYY-MM-DD HH24:MI:SS')) RETURNING id`,
			username, passwordHash,
		).Scan(&id)
		return id, err
	}
	res, err := sqlDB.Exec(
		`INSERT INTO web_users(username, password_hash, created_at) VALUES(?,?,`+now+`)`,
		username, passwordHash,
	)
	if err != nil {
		return 0, err
...
func openDB(dbURL string) (db.SQLDB, error) {
	return db.OpenSQLDB(dbURL)
}
