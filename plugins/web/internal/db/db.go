...
// OpenSQL returns a pooled *sql.DB (sqlite or postgres). Prefer OpenSQLDB when
// issuing `?` placeholders against Postgres (RBAC).
...
func sqlTypeFor(url, t string) string {
	ty := sqlType(t)
	if IsPostgres(url) && ty == "REAL" {
		return "DOUBLE PRECISION"
	}
	if IsPostgres(url) && ty == "BLOB" {
		return "BYTEA"
	}
	return ty
}
...
func Init(url, name string, fields any) (map[string]any, error) {
	...
	for _, c := range arr {
		...
		col := fmt.Sprintf(`"%s" %s`, colName, sqlTypeFor(url, ty))
		if colName == "id" && !hasPK {
			if IsPostgres(url) && sqlType(ty) == "INTEGER" {
				col = fmt.Sprintf(`"%s" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY`, colName)
			} else {
				col += " PRIMARY KEY"
...
func columnNames(c dbConn, tableName string, dbURL string) ([]string, error) {
	if IsPostgres(dbURL) {
		rows, err := c.Query(`
...
func Insert(url, tableName string, rows any, txnID ...string) (map[string]any, error) {
	...
	colsPresent, err := columnNames(c, tn, url)
	if err != nil {
		return nil, err
	}
...
// TableInfo returns column metadata for a table (SQLite or Postgres).
...
ORDER BY c.ordinal_position`, tn)
		...
		for rows.Next() {
			...
			var pk int
			if err := rows.Scan(&name, &dtype, &nullable, &pk); err != nil {
				return nil, err
			}
			if dtype == "" {
				dtype = "TEXT"
	...
	rows, err := c.Query(fmt.Sprintf(`PRAGMA table_info("%s")`, tn))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
